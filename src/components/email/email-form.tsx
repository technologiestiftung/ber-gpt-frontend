...
export const EmailForm: React.FC = () => {
	...
	return (
		<div
			className={`absolut w-full md:w-[640px] bottom-0 left-0 z-20 flex flex-col gap-y-4 bg-white px-5 pb-4 pt-1 md:relative md:border md:border-lighter-grey md:top-0 md:gap-y-4 md:rounded-md md:px-6 md:pt-4 md:shadow-md ${isEmailFormExpanded ? "top-[75px] rounded-t-2xl border-x border-t border-mid-grey" : ""}`}
		>
			<div
			...
			>
				...
				>
					...
					<div className="flex h-full flex-col gap-2 py-4 md:py-0 lg:w-2/5">
						...
						<textarea
							className={`h-20 w-full resize-none rounded bg-lighter-grey p-2 focus:border-blue-500 focus:outline-none`}
							name="previousMail"
				...
				<div className="flex flex-col gap-2">
					...
					<div
						className={`flex items-center gap-4 rounded bg-lighter-grey px-2 py-2 ${isEmailFormExpanded ? "shadow-none" : "shadow-md"} has-[:focus]:border-blue-500  md:shadow-none`}
					>
						<textarea
							className={`min-h-5 w-full bg-lighter-grey resize-none focus:outline-none md:h-10 md:max-h-40 md:resize-y ${isEmailFormExpanded ? "h-20" : "h-5"}`}
							name="message"
							required
